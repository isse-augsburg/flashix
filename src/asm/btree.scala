...
class btree_asm(var ADRT : address, var RT : znode, val apersistence : apersistence_interface)(implicit _algebraic_implicit: algebraic.Algebraic, _procedures_implicit: proc.Procedures) extends index_interface {
  ...
  override def format(VOLSIZE: Int, MAXINO0: Int, ERR: Ref[error]): Unit = {
    ...
    if (ERR.get == types.error.ESUCCESS) {
      ...
      if (ERR.get == types.error.ESUCCESS) {
        ...
        {
          val adr: Ref[address] = Ref[address](ADRT)
  ...
  override def index_entries(KEY: key, NAMES: stringset, ERR: Ref[error]): Unit = {
    ...
    {
      val rp: Ref[znode] = Ref[znode](RT)
      entries(types.key.dentrykey(KEY.ino, ""), null, ADRT, rp, DONE, NAMES, ERR)
      RT = rp.get
    }
  }
  ...
  override def index_truncate(KEY: key, N: Int, AS: address_set): Unit = {
    ...
    {
      val rp: Ref[znode] = Ref[znode](RT)
      truncate(types.key.datakey(KEY.ino, ((N + VFS_PAGE_SIZE) - 1) / VFS_PAGE_SIZE), null, ADRT, rp, DIRTY, DONE, AS, ERR)
      RT = rp.get
    }
  }
  ...
  override def recover(MAXINO0: Ref[Int], NL0: nat_list, OS0: nat_set, ERR: Ref[error]): Unit = {
    ...
    {
      val adr: Ref[address] = Ref[address](ADRT)
      apersistence.recover(adr, MAXINO0, NL0, OS0, ERR)
      ADRT = adr.get
    }
    RT = null
  }
  ...
  def traverse_root(MOD: modification, RP: znode, DIRTY: Ref[Boolean], EXISTS: Ref[Boolean], ADR: Ref[address], ERR: Ref[error]): Unit = {
    ...
    {
      val rp: Ref[znode] = Ref[znode](RT)
      traverse(MOD, RP, ADRT, rp, DIRTY, EXISTS, ADR, ERR)
      RT = rp.get
    }
    if (ERR.get == types.error.ESUCCESS) {
