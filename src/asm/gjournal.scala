...
class gjournal_asm(var DOSYNC : Boolean, var JMAXINO : Int, val JRO : nat_set, var JVALID : Boolean, var SQNUM : Int, val index : index_interface)(implicit _algebraic_implicit: algebraic.Algebraic, _procedures_implicit: proc.Procedures) extends aubifs_core_interface {
  ...
  def journal_transaction(NDLIST0: node_list, GNDLIST: group_node_list, SIZE: Ref[Int]): Unit = {
  ...
  def restore_log(NL: nat_list, LOG: address_list, ERR: Ref[error]): Unit = {
